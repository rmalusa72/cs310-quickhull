...
Dart_const_handle make_simplex(int simplex_dimension, CMap cm);
...
Dart_const_handle make_simplex(int simplex_dimension, CMap cm){
    ...
    if (simplex_dimension == 4){
        std::vector<Dart_const_handle> components;
        for(int i=0; i<simplex_dimension+1; i++){
            components.push_back(make_simplex(simplex_dimension-1, cm));
        }
        cm.sew<3>(components[1], components[2]);
        cm.sew<3>(cm.beta(components[1], 2), components[3]);
        cm.sew<3>(cm.beta(cm.beta(components[1], 1), 2), components[4]);
        cm.sew<3>(cm.beta(cm.beta(components[1], 0), 2), components[5]);
